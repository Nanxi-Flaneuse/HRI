...
# function for sphero cross activity
...
async def shake(droid, behavior = 'social'):
    if behavior == 'social':
        ...
        speak("Pick me up and follow my instructions! This is a fun exercise I used to do back at the Sphero Choral Music Society!")
        ...
        hands = ["left", "right"]
        for hand in hands:
            goal_shakes = random.randint(1, 5)
            speak(f"Hold me in your {hand} hand and shake {goal_shakes} times. Go!")
            # wait_for_shakes gives audible beep + LED feedback on each collision
            counted = await wait_for_shakes(droid, goal_shakes, timeout=30.0)  # 30s timeout safety
            if counted >= goal_shakes:
                speak(f"Great! I counted {counted} shakes with your {hand} hand.")
            else:
                speak(f"I only detected {counted} shakes for your {hand} hand. Let's try again later.")
            await asyncio.sleep(0.5)
...
def individual_testing():
    ...
    print("Scanning for toy…")
    toy = scanner.find_toy(toy_name="SB-C793")
    print(f"Found toy: {toy}")
    
    with SpheroEduAPI(toy) as droid:
        droid.register_event(EventType.on_collision)
        # function to test
